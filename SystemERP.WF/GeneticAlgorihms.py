...
'''
==============================================================
Rysowanie wykresu Gantta dla AlgorithmForSingleMachineScheduling
==============================================================
'''
def DiagramForSingleMachineScheduling(sequence_best):
        # Wczytanie danych z pliku Excel
    file_path = 'algorithmForOnePosition.xlsx'  # Ścieżka do pliku Excel
    df = pd.read_excel(file_path)
    p = df['Czas trwania'].tolist()
    ...
    df = []
    for j in j_keys:
        df.append(dict(Task='Machine', Start='2024-08-14 %s' % (str(j_record[j][0])), Finish='2024-08-14 %s' % (str(j_record[j][1])), Resource='Job %s' % (j + 1)))
    ...
    offline.plot(fig, filename='GA_flow_shop_scheduling_tardyjob.html', auto_open=True)
...
'''
==============================================================
Rysowanie wykresu Gantta dla AlgorithmGeneticFlowShopScheduler
==============================================================
'''
def DiagramGeneticFlowShopScheduler(sequence_best):
    import pandas as pd
    ...
    for m in m_keys:
        for j in j_keys:
            df.append(dict(Task=m, Start='2024-08-14 %s' % (str(j_record[(j, m)][0])), Finish='2024-08-14 %s' % (str(j_record[(j, m)][1])), Resource='%s' % (j)))
    ...
    offline.plot(fig, filename='DiagramGeneticFlowShopScheduler.html', auto_open=True)
